...
> ### 다형성(polymorphism)
> 
> 다형성이란 특정 기능을 **선언부분**과 **구현부분**으로 분리한 후 구현부분을 다양한 방법으로 만들어 선택해서 사용할 수 있게 하는 기능이다. 선언부분은 **인터페이스**라고 부르기도 한다.
> 
> 선언부분은 구현 코드가 전혀 없는 상태이며, 일종의 지켜야할 약속으로 채워진 규약 문서이고, 개발자는 문제를 해결하는 구현 코드를 선언부분에 맞게 구현하면 된다. 즉, 선언부분과 구현부분은 1:N 의 다형성 관계가 형성된다고 할 수 있다.
> 
> ### 다형성과 데이터 타입과의 관계
> 
> 자바스크립트에서는 데이터의 타입이 중요하게 사용되지는 않지만 일반 프로그래밍에서 데이터 타입은 매우 중요하다.
> 
> 일반 프로그래밍에서는 대부분 변수에 저장할 데이터 타입을 미리 작성해야하고, 이와 같은 개념은 함수와 메서드에서도 그대로 사용된다.
> 
...
> ### 자바스크립트의 다형성
>
> 일반 객체지향 프로그래밍에서의 다형성 관련 문법은 다음과 같다.
> 
> - 선언부분: 인터페이스(Interface)와 추상클래스(abstract class)
> - 구현부분: 클래스(class)
> 
> 자바스크립트는 인터페이스와 추상 클래스 그리고 엄격한 데이터 타입 체크 등의 다형성과 관련된 문법을 지원하지 않기 때문에, 그저 다형성을 지원한다는 가정하에 다형성스럽게 코드를 작성하면 된다.
> 
> 다형성 개념을 사용하면 좀더 좋은 프로그래밍이 가능해진다.
...
> ### 상속의 단점
> 
> 상속의 장점들은 상속을 적절히 사용했을 경우에만 해당하며, 상속을 잘못 사용하면 변화에 유연하지 않고, 오류를 내기 쉬운 소프트웨어가 된다.
> 
> 가장 큰 단점은 캡슐화를 깨뜨린다는 점이다. 상속은 상위 클래스의 구현이 하위 클래스에게 노출되기 때문에 캡슐화를 깨뜨리게 된다. 캡슐화가 깨지면 하위 클래스가 상위 클래스에 강하게 결합, 의존하게 되고, 강한 결합과 의존은 변화에 유연하게 대처하기 어렵게 만든다.

  * OOP의 합성(Composition)이란 무엇인가요? 합성이 상속에 비해 가지는 장점은 무엇일까요?

> ### OOP의 Composition(조합, 합성 ...)
> 
> 조합은 기존 클래스를 새로운 클래스의 구성 요소로 사용하는 것을 뜻한다.
> 
> 상위 클래스를 상속하는 것이 아니라, 새로운 클래스를 만들고 private 필드로 기존 클래스의 인스턴스를 참조하는 식으로 작업이 이루어진다.
> 
> ### 조합의 장점
> 
